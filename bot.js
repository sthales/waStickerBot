...
client
  .connect(authInfo, 20 * 1000) // connect or timeout in 20 seconds
  .then(([user, chats, contacts, unread]) => {
    ...
    client.setOnUnreadMessage((m) => {
      ...
      if (messageType === WhatsAppWeb.MessageType.text) {
        ...
        messageType === WhatsAppWeb.MessageType.location ||
        messageType === WhatsAppWeb.MessageType.liveLocation
      ) {
        ...
        client
          .decodeMediaMessage(m.message, "media_in_" + m.key.id)
          .then((meta) =>
            console.log(sender + " sent media, saved at: " + meta.filename)
          )
        ...
        client
          ...
          .then(() => {
            ...
            if (
              ...
              m.message.documentMessage.mimetype == "image/webp"
            ) {
